...
const getAll = (request, response) => {
    ...
    const {pagamento, bairro, delivery, show} = request.query
    ...
    if (show) {
        filtrados = filtrados.filter(estabelecimento => {
            return estabelecimento.show == (show == "true" ? true : false);
        });
    }
...
const updateDeslikes = (request, response) => {
    ...
    estabelecimentoFound.deslikes += 1;

    response.status(200).json(
        [
            {
                "mensagem" : "Deslikes do estabelecimento atualizado!",
                estabelecimentoFound
            }
        ]
    )
